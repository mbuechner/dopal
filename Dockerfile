...
# Add application, install and run Composer
# Move entrypoint script in place
COPY --chown=${RUN_USER}:${RUN_GROUP} scripts/docker-php-entrypoint-drupal.sh /usr/local/bin/docker-php-entrypoint-drupal
COPY --chown=${RUN_USER}:${RUN_GROUP} scripts/drupal-maintenance.sh /usr/local/bin/drupal-maintenance

# Copy Drupal, Composer and run Composer install
WORKDIR /var/www/html
COPY --chown=${RUN_USER}:${RUN_GROUP} web/ ./web/
COPY --chown=${RUN_USER}:${RUN_GROUP} config/sync/ ./config/sync/
COPY --chown=${RUN_USER}:${RUN_GROUP} composer.json composer.lock ./
COPY --from=composer:2 /usr/bin/composer /usr/bin/composer
RUN sudo -u ${RUN_USER} composer install --no-dev;
...
RUN \
    ...
    openssl req -new -key /etc/ssl/mykey.pem -out /etc/ssl/mycert.csr -subj "/C=DE/ST=DE/L=Frankfurt am Main/O=Deutsche Nationalbibliothek/OU=IT.DDB/CN=Dopal" && \
